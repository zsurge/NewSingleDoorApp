...
static int gUdpSock = -1;


static struct sockaddr_in gSeraddr={0};
static socklen_t gAddrlen=0;
...
static void sendToUdp(uint8_t *sendBuf,int sendLen);
...
static void sendToUdp(uint8_t *sendBuf,int sendLen)
{
	log_d ( "sendToUdp %s,addrlen =%d\r\n",inet_ntoa (gSeraddr.sin_addr),gAddrlen);

    /*将字串返回给发送端*/
    sendto ( gUdpSock,sendBuf,sendLen,0,( struct sockaddr* ) &gSeraddr,gAddrlen );
}
...
void deal_pc_procotol ( void )
{
	...
	recv_data_len=recvfrom ( gUdpSock,recv_data,RECV_DATA,0, ( struct sockaddr* ) &seraddr,&addrlen );
...
void sendToHost ( char CMD )
{
	...
	switch ( CMD )
	{
		...
		case SET_IP:
            ...
            sendToUdp(( uint8_t* ) sendBuff,len );
			vTaskDelay ( 50 );
			//4.重启
			NVIC_SystemReset();
			break;
